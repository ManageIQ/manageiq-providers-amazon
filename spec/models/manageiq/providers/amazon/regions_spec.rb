describe ManageIQ::Providers::Amazon::Regions do
  it "has all the regions" do
    ...
    VCR.use_cassette(described_class.name.underscore) do
      ...
      current_regions.reject! { |r| r[:region_name] == 'us-gov-west-1' || r[:region_name] == 'cn-north-1' }
  ...
  context "disable regions via Settings" do
    ...
    it "contains China region without it being disabled" do
      allow(Settings.ems.ems_amazon).to receive(:disabled_regions).and_return([])
      expect(described_class.names).to include("cn-north-1")
    end

    it "contains gov_cloud and China region without disabled_regions being set at all - for backwards compatibility" do
      allow(Settings.ems).to receive(:ems_amazon).and_return(nil)
      expect(described_class.names).to include("us-gov-west-1")
      expect(described_class.names).to include("cn-north-1")
    end

    it "does not contain some disabled gov_cloud" do
    ...
    it "does not contain disabled China region" do
      allow(Settings.ems.ems_amazon).to receive(:disabled_regions).and_return(['cn-north-1'])
      expect(described_class.names).not_to include('cn-north-1')
    end
  end
