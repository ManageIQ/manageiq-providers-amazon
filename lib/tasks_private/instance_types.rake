...
namespace 'aws:extract' do
  ...
  task :instance_types do
    ...
    GH_CACHE = ActiveSupport::Cache::FileStore.new(Rails.root.join('tmp', 'aws_cache', 'github'))
    ...
    discontinued_types, types_list = isolated do
      ...
      all_types, latest = versions.reduce([[], []]) do |(all, _latest), version|
    ...
    isolated do
      ...
      (new.keys | old.keys).each do |instance_type|
        ...
        [old_data, new_data].compact.each do |data|
          data.each_value { |v| v.sort! if v.kind_of?(Array) }
        end

        in_the_old = (old_data.to_a - new_data.to_a).to_h
        in_the_new = (new_data.to_a - old_data.to_a).to_h

        next if (attrs = in_the_old.keys | in_the_new.keys).empty?
        ...
        attrs.each do |attr|
          row_class =
            if !in_the_old.key?(attr) && in_the_new.key?(attr)
              added = true
              'added'
            elsif in_the_old.key?(attr) && !in_the_new.key?(attr)
              removed = true
              'removed'
            else
              changed = true
